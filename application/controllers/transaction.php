...
class transaction extends CI_Controller {
    ...
    function speedygetcontentdetail() {
        ...
        if (($this->page['islogin'] == true) && ($this->page['sessions'][0] == "SSC")) {
            $this->page['paymentaccounts'] = $this->msdp->ssc_getaccountinfo(array('emailaddress' => $this->page['sessions'][1]));
            //echo "<pre>"; print_r($this->page['paymentaccounts']); echo "</pre>";
        }
    ...
    function speedytrxorderbyemailcontent() {
        ...
        if (is_numeric($trxparams['CONTENTID']) && is_numeric($trxparams['SUBSCRIPTIONMONTH'])) {
            if ($this->input->valid_ip($trxparams['HOSTIP']) && $this->input->valid_email($trxparams['EMAILACCOUNT'])) {
                ...
                $this->page['error_mesg'] = "Data yang Anda masukkan tidak valid, silahkan cek data email yang dimasukkan." . "a" . $this->input->valid_ip($trxparams['HOSTIP']);
            }
            //print_r($trxparams);
        } else {
            $this->page['iserror'] = true;
        ...
        if (!$this->page['iserror']) {
            ...
            $this->email->bcc('sidiq.wahyudi@gmail.com');
...
powered by Webstore TSDC');
    ...
    function ssctrxbuycontent() {
        //echo "<pre>"; print_r($_POST); print_r($this->urisegments); echo "</pre>";
        $trxparams['CONTENTID'] = $this->urisegments['f_CONTENTID'];
        if ($this->page['islogin'] == true) {
            $trxparams['SSCACCOUNT'] = $this->page['sessions'][1];
            $trxparams['PAYMENTACCOUNT'] = $this->input->post('f_PAYMENTACCOUNT');
            $paymentaccounts = explode("|", strtoupper($trxparams['PAYMENTACCOUNT']));
            if (count($paymentaccounts) > 1) {
                $trxparams['PAYMENTACCOUNT_NUMBER'] = $paymentaccounts[1];
                $trxparams['PAYMENTACCOUNT_TYPE'] = $paymentaccounts[0];
                $trxparams['PAYMENTACCOUNT_ATTRIBUTE'] = $paymentaccounts[2];
            }
            $trxparams['HOSTIP'] = $this->input->ip_address();
            ...
            //check input validation
            if (is_numeric($trxparams['CONTENTID']) && $this->input->valid_email($trxparams['SSCACCOUNT']) && is_numeric($trxparams['PAYMENTACCOUNT_NUMBER']) && is_numeric($trxparams['SUBSCRIPTIONMONTH'])) {
                if ($this->input->valid_ip($trxparams['HOSTIP']) && $this->input->valid_email($trxparams['EMAILACCOUNT'])) {
                ...
                $this->page['iserror'] = true;
                $this->page['error_mesg'] = "Periksa kembali data <b>Email Anda </b> dan <b>Akun Pembayaran</b> yang Anda masukkan.";
            }
            //get content information
            $contentparams = $this->mmasterdata->gettrxcontentparam($trxparams['CONTENTID']);
            if (!$contentparams) {
            ...
            if (!$this->page['iserror']) {
                if ($trxparams['PAYMENTACCOUNT_TYPE'] == 'SPEEDY') {
            ...
            if (!$this->page['iserror']) {
                ...
                $subsstartdate = "'" . date('Y-m-d H:i:s') . "'";
                if (strtoupper($trxparams['PAYMENTACCOUNT_ATTRIBUTE']) == "POSTPAID") {
                    $subsenddate = 'null';
                    if ($trxparams['SUBSCRIPTIONMONTH'] != 0) {
                        //secho $subsstartdate= date('Y-m-d H:i:s')."<br />"; //yyyy-mm-dd hh:mm:ss
                        //echo substr($subsstartdate,0,4)."<br />";
                    ...
                    $subsenddate = "'" . date('Y-m-d') . " 23:59:59'";
                }
                $trxparams['SUBSCRIPTIONID'] = $this->mmasterdata->insertsubscriptiontrxssc(
                                $contentparams['CONTENTID'], $contentparams['SUBSCRIPTIONTYPE'], $trxparams['SUBSCRIPTIONSTATUS'], 'OLSTORE-SPEEDY', $trxparams['SSCACCOUNT'], $trxparams['PAYMENTACCOUNT_NUMBER'], $trxparams['PAYMENTACCOUNT_TYPE'], $trxparams['PAYMENTACCOUNT_ATTRIBUTE'], $trxparams['EMAILACCOUNT'], '', $trxparams['HOSTIP'], $trxparams['SUBSCRIPTIONMONTH'], $subsstartdate, $subsenddate, '');
                $trxparams['ACTIVATIONID'] = $this->mmasterdata->gettrxactivationid($trxparams['SUBSCRIPTIONID']);
            }
            ...
            if (!$this->page['iserror']) {
                ...
                if ($result == 1) {
                    ...
                    if ($this->mmasterdata->updatesubscriptiontrxssc($trxparams) != 1) {
                        $this->page['iserror'] = true;
            ...
            //echo '<br />RC: '.$this->page['iserror'];
            //echo '<br />MSG: '.$this->page['error_mesg'];
            $this->load->view('store/include/div-ssc-trx-result-box', $this->page);
        }
    }
