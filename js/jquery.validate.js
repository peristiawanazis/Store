...
$(document).ready(function() {
	$("#dgform").validate({
		rules: {
			...
			f_birthplace: {required: true,minlength: 4},
			f_email: {required: true,email: true},
		},
		messages: {
			f_birthplace: "Masukkan tempat lahir anda dengan benar",
			f_name: {required: "Masukkan Nama Anda Dengan Benar",minlength: "Nama anda tidak boleh kurang dari 2 angka"},
			f_birthplace: {required: "Tempat lahir tidak boleh kosong",minlength: "Tempat lahir tidak boleh kurang dari 4 kata"},
			f_email: "Alamat email anda tidak benar",
		},
	 errorPlacement: function(error, element) {error.appendTo(element.parent("td"));
	 }
   });
   $("#dgformcontent1").validate({
		rules: {
			f_name: {required: true,minlength: 2},
			f_birthplace: {required: true,minlength: 4},
			f_email: {required: true,email: true},
